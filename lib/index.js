...
import stateStore from '../resources/state-store.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));

const GATEWAY_PORT = parseInt(process.env.DSWEB_PORT || '5688', 10) || 5688;
const RESOURCES_DIR = path.join(__dirname, '..', 'resources');
const GATEWAY_FILE = path.join(RESOURCES_DIR, 'dsweb-gateway.js');
const BASE_DIR = stateStore.resolveStateDir();
const TOKEN_FILE = path.join(BASE_DIR, 'gateway-token');
...
function gatewayAlive() {
  ...
  return fetch(`http://127.0.0.1:${GATEWAY_PORT}/v1/models`, { headers: { Authorization: 'Bearer ' + token } })
    .then((r) => r.ok)
    .catch(() => false);
}
...
async function doEnsureGateway() {
  ...
  stateStore.ensurePrivateDir(BASE_DIR);
  ...
  gwStartedByUs = true;
  let errTail = '';
  /* 网关 stdout 转发到 DSH stderr（网关 log 用 console.log，driver 日志也经此转发） */
  cp.stdout.setEncoding('utf8');
  cp.stdout.on('data', (c) => { process.stderr.write(c); });
  cp.stderr.setEncoding('utf8');
  cp.stderr.on('data', (c) => { errTail = (errTail + c).slice(-2000); process.stderr.write(c); });
