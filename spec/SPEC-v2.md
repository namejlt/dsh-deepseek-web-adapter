...
> 状态：**P0 已实现并通过测试（tests/test-account-pool.js 61/61）；2026-08 完备性审计后新增 §11 稳定性加固**
...
4. **易用性**：`/setup` 一键配置引导；`/accounts` 管理 API；启动即登录引导；只读状态页
5. **单账号零回归**：只有 1 个账号时行为与 v1 完全一致（含会话亲和并发模型）

### 1.3 非目标（Non-Goals）

- 不存储账号密码/不做自动填充凭据（安全红线，见 §8 风险 R5）
- 不做卡片 UI（在开发仓库 `dsweb-plugin/client-llm.js`，本包只提供 HTTP 面）
...
│  /setup /accounts /health(HTML) （新增 HTTP 面）              │
